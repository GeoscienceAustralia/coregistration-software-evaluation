...
def make_mosaic(
    ...
    if universal_masking:
        ...
        if cluster_masks:
            ...
            edge_scene_count = [0] * len(mask_per_cluster)
            if masking_edge_scene_count:
                ...
                for i, cluster_masks in enumerate(masks_per_cluster):
                    ...
                    start_col = int((data_cols.min() + data_cols.max()) / 2)
                    start_row = 0
                    sum_data = 0
                    num_masks_per_row = []
                    while (
            ...
            for i, warp in enumerate(warps):
                if masks_dict[i] != -1:
                    ...
                    warps_idx = idx_list[masks_dict[i]]
            ...
            if masking_edge_scene_count:
                data_cols = np.where(
                    (np.all(universal_mask, axis=2) == 0)
                    if len(universal_mask.shape) == 3
                    else universal_mask == 0
                )[1]
                start_col = int((data_cols.min() + data_cols.max()) / 2)
                start_row = 0
                sum_data = 0
                num_masks_per_row = []
                while sum_data != len(masks) and start_row < universal_mask.shape[0]:
                    have_data = []
                    for mask in masks:
                        have_data.append(
                            np.logical_or.reduce(mask[start_row, start_col])
